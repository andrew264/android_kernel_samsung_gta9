 ...
 */
#define RTNL_LOCK_MAX_HOLD_TIME_MS	500	/* 500ms */
...
static void rtnl_get_btrace(struct task_struct *who)
{
	...
	expires = jiffies + (RTNL_LOCK_MAX_HOLD_TIME_MS * HZ / 1000);
...
static void rtnl_print_btrace(struct timer_list *unused)
{
	...
	pr_info("rtnetlink: %s[%d][%c] hold rtnl_lock more than %d msec, start time: %llu\n",
		rtnl_instance.task->comm,
		rtnl_instance.pid,
		task_state_to_char(rtnl_instance.task),
		RTNL_LOCK_MAX_HOLD_TIME_MS,
...
static void rtnl_relase_btrace(void)
{
	...
	if (rtnl_instance.end_time - rtnl_instance.start_time > 
		(RTNL_LOCK_MAX_HOLD_TIME_MS * NSEC_PER_SEC / 1000)) {
